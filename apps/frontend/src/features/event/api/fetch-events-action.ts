import { FetchEventListQueryParametersDTO, FetchEventListItemResponseDTO, FetchEventListItemResponseSchema } from "@project/shared";
...
export const fetchEventsAction = async (queryParameters: FetchEventListQueryParametersDTO): Promise<FetchEventListItemResponseDTO[]> => {
    ...
    const parsedResponse = response.map(event => FetchEventListItemResponseSchema.parse(event));
