...
export class EventService {
  ...
  async fetchEvents(params: FetchEventQueryParametersDTO): Promise<FetchEventResponseDTO[]> {
    const events = await this.eventsRepository.findEvents(params);

    return events.map((event) => ({
      eventId: event.id,
      imageUrl: event.imageUrl ?? 'https://img.magnific.com/free-photo/beautiful-view-sunset-sea_23-2148019892.jpg?size=626&ext=jpg',
      title: event.title,
